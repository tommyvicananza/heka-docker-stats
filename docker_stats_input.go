...
func (input *DockerStatsInput) Run(runner pipeline.InputRunner,
	...
	for {
		...
		for _, container := range containers {
			...
			memLimit, err := message.NewField("MemoryLimit", uint64(mstats.MemLimit), "")
			fmt.Println(err)
			...
			//netInput, _ := message.NewField("NetworkInput", uint64(mstats.NetworkRx), "")
			//pack.Message.AddField(netInput)
			//netOutput, _ := message.NewField("NetworkOutput", uint64(mstats.NetworkTx), "")
			//pack.Message.AddField(netOutput)
			//blockInput, _ := message.NewField("BlockInput", uint64(mstats.BlockRead), "")
			//pack.Message.AddField(blockInput)
			//blockOutput, _ := message.NewField("BlockOutput", uint64(mstats.BlockWrite), "")
